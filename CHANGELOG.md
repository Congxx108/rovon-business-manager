...
## 2026-09-18 — WhatsApp1～4 有效潜客统计口径

- 生产 migration 已执行：180条记录、120条总增加人工值保留，60条自动值变化，总和6932→5755；原累计值、override、群标记及群增加结果指纹一致。lint、build、独立 TypeScript 检查通过。

- WhatsApp1～4 是最终有效潜客池；`total_increase` 只统计四个 WhatsApp 的每日增量；Facebook 后台潜客保留为辅助指标，不参与总潜客增加计算。
- 新增 `20260918025512_add_whatsapp4_whatsapp_only_leads.sql`，WhatsApp4 与其他账号字段约束一致，历史默认0，并通过现有函数统一重算历史；首条自动0、override含0优先、群数据/reset逻辑保留。
- 快速录入、新增、编辑、历史表格、类型、数据检查/清理预览、CSV模板/预览/导出接入WhatsApp4；核心页面前置WhatsApp，Dashboard明确新口径并继续按最终增加数和日期范围汇总。
- FB增加与占比在查询层派生，无新持久化字段；列表读取额外前序记录，分母非正、非有限、FB负增长或超过100%时显示 `-`。
- 旧CSV缺失/空白WhatsApp4时，已有日期保留原值、新日期为0。保留CSV增加数默认不勾选，已有人工值（含0）和备注始终保留；显式选择只填补尚无人工值的字段。
- 隔离PGlite验证：示例总增加100、FB增加25/占比25%，历史修改/日期移动、首条、override0、群标记、五个录入action、CSV兼容/保护、Dashboard日期筛选与页面导出。
