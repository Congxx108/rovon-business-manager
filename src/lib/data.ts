import { withFacebookMetrics, type DailyLeadWithFacebookMetrics } from "@/lib/daily-lead-metrics";
...
export async function getDailyLeads(limit = 60): Promise<DataResult<DailyLeadWithFacebookMetrics[]>> {
  if (!isSupabaseConfigured()) return emptyResult([]);
  ...
  if (error) return emptyResult([], error.message);
  return emptyResult(withFacebookMetrics((data ?? []) as DailyLead[]).slice(0, limit));
}
...
export async function getDataCheckData(): Promise<DataResult<DataCheckData>> {
  ...
  const [ordersResult, customersResult, leadsResult] = await Promise.all([
    ...
    supabase
      .from("daily_leads")
      .select("stat_date,facebook_leads,whatsapp1,whatsapp2,whatsapp3,whatsapp4,handbag_group,backpack_group,total_increase,handbag_group_increase,backpack_group_increase,total_increase_override,handbag_group_increase_override,backpack_group_increase_override,is_handbag_group_reset,is_backpack_group_reset"),
  ]);
  ...
  return emptyResult({
    ...
    dailyLeads: [
      ...
      metric(
        "五个累计字段为空的记录数",
        leads.filter((lead) => lead.facebook_leads === null || lead.whatsapp1 === null || lead.whatsapp2 === null || lead.whatsapp3 === null || lead.whatsapp4 === null).length,
        "error",
      ),
      metric(
        "群人数为空的记录数",
    ...
    importReview: {
      ...
      recentDailyLeads: [...leads].reverse().slice(0, 7).map((lead) => ({
        ...
        whatsapp4: Number(lead.whatsapp4 ?? 0),
...
export type DataCleanupPreview = {
  ...
  testDailyLeads: Pick<DailyLead, "id" | "stat_date" | "facebook_leads" | "whatsapp1" | "whatsapp2" | "whatsapp3" | "whatsapp4" | "handbag_group" | "backpack_group">[];
...
export async function getDataCleanupPreview(): Promise<DataResult<DataCleanupPreview>> {
  ...
  const [ordersResult, leadsResult] = await Promise.all([
    ...
    supabase
      .from("daily_leads")
      .select("id,stat_date,facebook_leads,whatsapp1,whatsapp2,whatsapp3,whatsapp4,handbag_group,backpack_group")
